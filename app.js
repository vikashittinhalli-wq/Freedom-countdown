...
// Homebound v4.0 experience layer
(function(){
  const HB_KEY='homeboundV4';
  function hbLoad(){try{return JSON.parse(localStorage.getItem(HB_KEY)||'{}')}catch(e){return {}}}
  function hbSave(v){localStorage.setItem(HB_KEY,JSON.stringify(v))}
  const hb=hbLoad();
  window.addEventListener('load',()=>setTimeout(()=>document.getElementById('homeboundSplash')?.classList.add('hide'),1450));
  const hour=new Date().getHours();
  const greeting=hour<12?'Good morning':hour<18?'Good afternoon':'Good evening';
  const g=document.getElementById('homeGreeting'); if(g) g.textContent=greeting+', welcome home';
  const messages=['You are building this life together.','Small steps become beautiful chapters.','Today counts, even when progress feels quiet.','Your future home begins with what you do today.'];
  const msg=document.getElementById('dailyHomeboundMessage'); if(msg) msg.textContent=messages[new Date().getDate()%messages.length];
  const note=document.getElementById('dreamNote'); if(note) note.value=hb.dreamNote||'';
  document.getElementById('saveDreamNote')?.addEventListener('click',()=>{hb.dreamNote=note.value.trim();hbSave(hb);const st=document.getElementById('dreamNoteStatus');st.textContent='Dream note saved.';setTimeout(()=>st.textContent='',1800)});
  document.querySelectorAll('.dream-card').forEach(card=>{
    const name=card.dataset.dream;
    if((hb.completedDreams||[]).includes(name)) card.classList.add('completed');
    card.addEventListener('click',()=>{
      hb.completedDreams=hb.completedDreams||[];
      if(hb.completedDreams.includes(name)){hb.completedDreams=hb.completedDreams.filter(x=>x!==name);card.classList.remove('completed')}
      else{hb.completedDreams.push(name);card.classList.add('completed'); if(window.premiumConfetti) premiumConfetti();}
      hbSave(hb);
    });
  });
  function refreshExtras(){
    let finance={};try{finance=JSON.parse(localStorage.getItem('freedomCountdownData')||'{}')}catch(e){}
    const original=35985.64;
    const current=Number(finance.cc1||0)+Number(finance.cc2||0)+Number(finance.zipPlus||0)+Number(finance.zipMoney||0);
    const pct=Math.max(0,Math.min(100,(original-current)/original*100));
    const dreamPct=document.getElementById('dreamFreedomPct');if(dreamPct)dreamPct.textContent=pct.toFixed(0)+'%';
    const tree=document.getElementById('gardenTree');if(tree)tree.style.setProperty('--tree-scale',(0.48+pct/100*.52).toFixed(2));
    const stage=document.getElementById('gardenStage');if(stage)stage.textContent=pct<15?'Planting the first seeds':pct<50?'Strong roots are forming':pct<85?'Your garden is flourishing':'Freedom is in full bloom';
    const mem=document.getElementById('memoryTimeline');if(mem){
      const events=[];
      events.push({t:'Homebound began',d:'A shared journey toward home, freedom and beautiful memories.'});
      (finance.history||[]).slice(-3).reverse().forEach(h=>events.push({t:new Date(h.date).toLocaleDateString('en-AU',{day:'numeric',month:'short',year:'numeric'}),d:'Progress saved — another step closer.'}));
      if(Number(finance.zipMoney||1)<=0)events.push({t:'Zip Money cleared',d:'A major milestone became part of your story.'});
      mem.innerHTML=events.map(e=>'<div class="memory-event"><strong>'+e.t+'</strong><span>'+e.d+'</span></div>').join('');
    }
  }
  refreshExtras();
  document.addEventListener('click',e=>{if(e.target.closest('#applyPayments,#saveBalances'))setTimeout(refreshExtras,150)});
})();
